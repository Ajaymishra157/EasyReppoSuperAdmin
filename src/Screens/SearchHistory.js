...
const SearchHistory = () => {
  ...
  const SearchHistoryApi = async (fromdate, tilldate, pageNumber = 1) => {
    console.log('Fetching page xxxxxxxx:', pageNumber);
    ...
    try {
      ...
      console.log('Fetching page2:', pageNumber, 'from', fromdate, 'to', tilldate, 'rent Agency Id', rentAgencyId);
      ...
      const result = await response.json();
      console.log("success hua search api main", result);
  ...
  const handleSearch = async (inputText, pageNumber = 1) => {
    ...
    try {
      ...
      if (result.code == 200) {
        ...
        // ✅ 404 ya koi bhi error aaye toh allLoaded true karo
        if (result.code === 404 || pageNumber > 1) {
          console.log('Search error - setting allLoaded to true');
          setAllLoaded(true);
        }
      }
    } catch (error) {
